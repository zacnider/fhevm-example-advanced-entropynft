...
async function main() {
  ...
  console.log(`\nDeploying EntropyNFT...`);
  
  const ContractFactory = await hre.ethers.getContractFactory("EntropyNFT");
  const contract = await ContractFactory.deploy(ENTROPY_ORACLE_ADDRESS, deployer.address);
  
  await contract.waitForDeployment();
  const address = await contract.getAddress();
  
  console.log(`\n✅ EntropyNFT deployed to:`, address);
  console.log(`\n📋 Deployment Details:`);
  console.log(`   Network: ${hre.network.name}`);
  console.log(`   Deployer: ${deployer.address}`);
  console.log(`   Contract: EntropyNFT`);
